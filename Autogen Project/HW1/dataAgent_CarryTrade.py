...
async def process_chunk(chunk, start_idx, total_records, model_client, termination_condition):
    ...
    處理單一批次資料：
      ...
      - 請 MultimodalWebSurfer 代理人利用外部網站搜尋功能，
        搜尋公債殖利率波動的新聞與資訊（例如政府利率政策、外部環境影響等），
        並將搜尋結果納入建議中。
      - 收集所有回覆訊息並返回。
    """
    # 將資料轉成 dict 格式
    ...
    prompt = (
        ...
        f"以下為該批次資料:\n{chunk_data}\n\n"
        "請根據以上資料進行分析，並提供完整的市場環境分析與利差交易建議。"
        "其中請特別注意：\n"
        "  1. 分析公債殖利率走勢；\n"
        "  2. 請 MultimodalWebSurfer 搜尋外部網站，找出公債殖利率波動的新聞與資訊（例如政府利率政策、外部環境影響等）完成市場分析，\n"
        "     並將搜尋結果整合進回覆中；\n"   
        "  3. 最後請提供具體的利差交易建議（包含買入賣出的時間點）、風險和相關參考資訊。\n"
